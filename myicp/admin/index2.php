...
if(!empty($_GET["password"]) && $_GET["password"] == $password) {
	if(!empty($_GET["fail"])) {
		searchAndMoveValue("../idinfo.json", "fail.json", "id", $_GET["fail"]);
		searchAndMoveValue2("../id.json", "fail.json", $_GET["fail"]);
	}
	...
	foreach($data as $item) {
		echo "ID: {$item["id"]}<br>域名: {$item["domain"]}<br>介绍: {$item["description"]}<br>站长: {$item["master"]}<br>邮箱: {$item["email"]}<br><a href=\"?fail={$item["id"]}&password={$_GET["password"]}\">删除</a><br><br>";
	}
} else {
